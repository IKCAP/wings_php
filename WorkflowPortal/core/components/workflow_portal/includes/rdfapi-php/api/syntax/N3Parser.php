...
function str2unicode_nfc($str=""){
  ...
  /*function getSpan(&$list, $start, $end) {
    ...
    $list = $this->array_concat($pre,$post);
    return array_slice($list, $start,$end-$start);
  }*/
  function getSpan(&$list, $start, $end) {
	 return array_splice($list, $start, $end-$start);
  }
...
//var_dump($newres[0]);
   ...
   * @returns array
   **/
  function getPrefixesAndBase($list) {

    $prefixes=array();
    $ns=1;
    $name=2;
	 $list= array_slice($list,0);
    while (($pos = array_search('@prefix', $list)) !== FALSE) {
		$binding = $this->getSpan($list, $pos, ($pos+4)); # processes the prefix tokens
    ...
    while(in_array('@base', $list)) {
      $pos = array_search('@base', $list);
		$binding = $this->getSpan($list, $pos, ($pos+3)); # processes the base token
		$this->baseURL = substr($binding[$ns],1,-1);
    }
                ...
                if ($bLiteral) {
                    ...
                    if (substr($list[$i], 0, 3) == $tripleBase) {
                        if (substr($list[$i],-3,3) == $tripleBase) {
                            ...
                            //$lit=preg_replace("/[^\\]/" . $chBase, "\\" . $chBase, $lit);
  ...
  function getStatements($list) {
    ...
    while (in_array('.', $list)) {
      ...
      $statement=$this->getSpan($list, 0, $pos+1);
  ...
  function getPovs($list) {
    ...
    while (in_array(';', $list)) {
      ...
      $pov = $this->getSpan($list, $pos[0], $pos[1]);
  ...
  function getObjs($list) {
    ...
    while (in_array(",",$list)) {
      ...
      $obj=$this->getSpan($list, $pos, ($pos+$get_array_fields));
...
//    $map=array();
  ...
  function n3tolist($s) {
   ...
   $t = $this->filterWs($this->toke($s)); # tokenize the stream, and filter whitespace tokens
    ...
    $r=$this->getPrefixesAndBase($t); # get the prefix directives, and add to a dict
    $prefixes=$r[0];
    $t=$r[1];
    if ($this->debug) {
      print "Prefixes:\n";
...
print "<pre>".preg_replace('/[<>]/','',print_r($obj, true))."</pre>";
}
?>
