/**
	...
	* @cfg {String} filesizelimit The maximum file size allowed, default "3 GB"
...
Ext.ux.swfbtn = Ext.extend(Ext.Button, { 
	constructor: function(cfg) {
		Ext.apply(this, cfg);
		this.text = cfg.text || "Add Slides";
		this.iconCls = cfg.iconCls || null;
		this.title = cfg.title || "File Upload Queue";
		...
		if(!Ext.ModelManager.isRegistered('SWFFileStoreModel'))
			Ext.define('SWFFileStoreModel', {
				extend: 'Ext.data.Model',
  	  			fields: ['id', 'index', 'name', 'datatype', 'size', 'type', 'creationdate', 'modificationdate', 'filestatus']
			});

	   var filestore = new Ext.data.JsonStore({
			model: 'SWFFileStoreModel',
    		data: [],
    	});
			...
			listeners: {
				'render': function(btn) {
		    		...
		    		var settings = {
						...
						file_size_limit: cfg.filesizelimit || "3 GB",
				    ...
				    function myCustomFileQueuedHandler(f){
				    	var myrec = new SWFFileStoreModel({
				    		id: f.id,
				    		index: f.index,
				    		name: f.name,
				    		size: f.size,
				    ...
				    function myCustomUploadSuccessHandler(file, server_data, receivedResponse){
				    	lastbytecount = 0;
				    	++swffilessent;
			    		if(cfg.success_callback_fn) { cfg.success_callback_fn.call(this, file, server_data) }
				    	if(swffilessent == filestore.getCount()) {
			    			...
			    			if(cfg.allcomplete_callback_fn) { cfg.allcomplete_callback_fn.call(this, file, server_data) }
				    	}
		...
		var grid = new Ext.grid.GridPanel({
			...
			store: filestore,
			columns: [
				{header: 'File', width: 200, dataIndex: 'name'},
				//{header: 'DataType', width: 200, dataIndex: 'datatype'},
				{header: 'Size', width: 110, dataIndex: 'size'},
				//{header: 'Created', width: 200, dataIndex: 'creationdate'},
		...
		this.on('click', function(obj, e, opts) { gridwin.show(); }, this);
	}
});
