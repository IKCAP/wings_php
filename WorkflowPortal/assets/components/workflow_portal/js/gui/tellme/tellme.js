...
function matchComponentRoles(rs, roles, isInput, dtparents) {
	...
	for(var i=0; i<rs.length; i++) {
		...
		for(var j=0; j<tmp.length; j++) {
			tmp2.push(tmp[j]);
			var types = tmp[j].types;
			if(!match) {
				var typeMatch = true;
				for(var k=0; k<types.length; k++) {
					if(isInput && !typeSubsumesType(types[k], r, dtparents)) 
						typeMatch = false;
					else if(!isInput && !typeSubsumesType(r, types[k], dtparents)) 
						typeMatch = false;
				}
				match = typeMatch;
				if(match) tmp2.pop();
...
function flattenData(data_root) {
	...
	while(tmp.length) {
		var d = tmp.pop();
		data[d.id] = {name:d.id, type:(d.type ? 'Datatype':'Data')};
		if(d.children)
...
function parseTellMeInstruction(panel, datalist, complist, dtparents, e) {
	...
	for(var v in gp.editor.template.variables) 
		tdatalist[v] = {name:v, type:'Variable'};
...
function getTellMePanel(guid, tid, tname, tabPanel, mainPanel, opts, store, run_url, op_url) {
	...
	var complist = flattenComponents(store.components);
	var datalist = flattenData(store.data);
	var dtparents = getDatatypeParentMap(store.data);
...
function getPortNameForDatatype(datatype, roles, dtparents) {
	for(var i=0; i<roles.length; i++) {
		...
		if(r.param) continue;
		var typeMatch = true;
		for(var k=0; k<r.types.length; k++) {
			if(!typeSubsumesType(r.types[k], datatype, dtparents)) 
				typeMatch = false;
		}
		if(typeMatch) return r.role;
	}
	return null;
};
...
function getPortDatatype(portname, roles) {
	for(var i=0; i<roles.length; i++) {
		var r = roles[i];
		if(r.param) continue;
		if(r.role == portname && r.types.length) return r.types[0];
	}
	return null;
};
...
function addNodeIOToTemplate(n, data, isInput, template, cbyid, dtparents, ipvars, opvars, seln, selv, ind) {
	var port;
	var c = cbyid[n.component];
	if(!c) return null;
	...
	if(data.prop && data.val) {
		template.setVariablePropertyValue(v, 'dcdom:'+data.prop, data.val);
	}

	if(newvariable) {
		isInput ? template.addInputLink(port, v) : template.addOutputLink(port, v);
		if(data.role == 'Datatype') 
			template.setVariableType(v, data.name);
		else 
...
function addNodeInputToTemplate(n, ip, template, cbyid, dtparents, ipvars, opvars, seln, selv, ind) {
	return addNodeIOToTemplate(n, ip, true, template, cbyid, dtparents, ipvars, opvars, seln, selv, ind); 
}

function addNodeOutputToTemplate(n, op, template, cbyid, dtparents, ipvars, opvars, seln, selv, ind) {
	return addNodeIOToTemplate(n, op, false, template, cbyid, dtparents, ipvars, opvars, seln, selv, ind); 
}
...
function mergeVariablesInTemplate(xv1, xv2, template, cbyid, dtparents, seln, selv) {
...
function applyBeamerActionsToTemplate(template, tasks, cbyid, dtparents, seln, selv, index, ipvars, opvars) {
	...
	for(var i=index; i<tasks.length; i++) {
		...
		if(fn == "C_AddNode") {
			...
			n.setPortRule('S', n.pruleOp); // S type for TellMe Nodes
			...
			if(fn == "C_AddNodeWithInput" || fn == "C_AddNodeWithInputOutput") {
				var ip = args['ip'];
				if(!ip) return null;
				var v = addNodeInputToTemplate(n, ip, template, cbyid, dtparents, ipvars, opvars, seln, selv);
				if(!v) return null;
				//ipvars.push(v);
			}
			if(fn == "C_AddNodeWithOutput" || fn == "C_AddNodeWithInputOutput") {
				var op = args['op'];
				if(!op) return null;
				var v = addNodeOutputToTemplate(n, op, template, cbyid, dtparents, ipvars, opvars, seln, selv);
				if(!v) return null;
				//opvars.push(v);
			}
			if(fn == "C_AddNodeWith2Inputs") {
				...
				var v1 = addNodeInputToTemplate(n, ip1, template, cbyid, dtparents, ipvars, opvars, seln, selv);
				var v2 = addNodeInputToTemplate(n, ip2, template, cbyid, dtparents, ipvars, opvars, seln, selv, 1);
				if(!v1 || !v2) return null;
				//ipvars.push(v1);
				//ipvars.push(v2);
			}
		...
		if(fn == "V_MergeVariables") {
			...
			var v2 = args['v2'];
			if(!v1 || !v2) return null;
			var v = mergeVariablesInTemplate(v1, v2, template, cbyid, dtparents, seln, selv);
			if(!v) return null;
		...
		if(n) {
    		...
    		var tightQ = getTightenedTemplates(template, n, cbyid, dtparents);
			for(var j=0; j<tightQ.length; j++) {
				// Apply rest of the beamer action tasks on these tightened templates (notice index is now i+1)
				var tQ = applyBeamerActionsToTemplate(tightQ[j], tasks, cbyid, dtparents, seln, selv, i+1, ipvars, opvars);
				if(!tQ) continue;
...
function getTightenedTemplates(template, node, cbyid, dtparents) {
	var vms = {};
	var cvms = {};
	var vmaps = {};
	...
	for(var i=0; i<ilinks.length; i++) {
		...
		if(!l.fromPort) {
			...
			if(!tmp.length) {
				...
				var cmps = findMatchingComponents(cbyid, [], [type], dtparents);
				...
				for(var j=0; j<cmps.length; j++) {
					...
					for(var k=0; k<c.outputs.length; k++) {
						var couttype = c.outputs[k].types[0];
	...
	for(var i=0; i<olinks.length; i++) {
		...
		vms[v.id] = [];
		if(!l.toPort) {
			var tmp = findMatchingVariablesInTemplate(template, node, v, false, dtparents);
			for(var j=0; j<tmp.length; j++) {
				var map = v.id+","+tmp[j];
				var rmap = tmp[j]+","+v.id;
				if(vmaps[rmap]) continue;
	...
	for(vid in vms) {
		...
		for(var j=0; j<len; j++) {
			...
			for(var i=0; i<mvids.length; i++) {
				...
				var v = mergeVariablesInTemplate({name:vid}, {name:mvids[i]}, tmp2, null, dtparents, null, null);
				if(v) {
					tQ.push(tmp2);
					nochange = false;
				}
	...
	for(vid in cvms) {
		...
		for(var j=0; j<len; j++) {
			...
			for(var i=0; i<cmvids.length; i++) {
				...
				var v = mergeVariablesInTemplate({name:mv.id}, {name:xc.vid}, tmp2, null, dtparents, null, null);
				if(!v) continue;
				...
				if(!mv) continue;
				var v = mergeVariablesInTemplate({name:mv.id}, {name:vid}, tmp2, null, dtparents, null, null);
				if(!v) continue;
...
function applyBeamerMatches(text, matches, complist, tdatalist, dtparents, panel, gp, ed) {
	...
	for(var i=0; i<matches.length; i++) {
		...
		var ts = applyBeamerActionsToTemplate(t, tasks, complist, dtparents, seln, selv);
		if(ts && ts.length) {
			num += ts.length;
			tQ = tQ.concat(ts);
		}
...
/*function beamerSelectionHandler(gp, ed, text, actions, cbyid, panel) {
	if(!text) return;
	var hp = panel.histories.getLayout().activeItem;
	var hpTree = hp.getComponent('tellmeHistoryTreePanel');
	var hpDetail = hp.getComponent('tellmeHistoryDetailPanel');
	...
	var tQ = applyActionsToTemplate(t, a, cbyid, 0);
