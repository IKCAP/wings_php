...
var Canvas = function(elid) {
	var defaultCanvas = {
		...
		getScrollXY : function (canvasOnly) {
			...
			var el = this.getEl();
			scrOfY = (canvasOnly ? 0 : Math.max(document.body.scrollTop,document.documentElement.scrollTop)) + parseInt(el.parentNode.parentNode.scrollTop);
			scrOfX = (canvasOnly ? 0 : Math.max(document.body.scrollLeft,document.documentElement.scrollLeft)) + parseInt(el.parentNode.parentNode.scrollLeft);
		...
		translateEventCoordsToCanvasCoords : function (pageX, pageY) {
			...
			// Adjust mouseX and Y with the current X and Y of the canvas. This will give us a 
		...
		fireEvent : function (event) {
			...
			for(i in canvas.ObjectManager.objects) {
				if(typeof(canvas.ObjectManager.objects[i]) == "function") continue;
	...
	var defaultObjectManager = {
		...
		findObject : function (objectID, type) {
			...
			for(i in this.objects) {
				if(typeof(this.objects[i]) == "function") continue;
	...
	var defaultLayerManager = {
		...
		findLayer : function (layerID) {
			...
			for(i in this.layers) {
				if(typeof(this.layers[i]) == "function") continue;
		...
		findThread : function(threadID) {
			...
			if(typeof(this.threads) == "object" && (this.threads instanceof Array)) {
				for(i in this.threads) {
					if(typeof(this.threads[i]) == "function") continue;
