...
public class PortSetRuleHandler {
	...
	public static PortBindingList normalizePortBindings(Node n, Template t) {
		...
		for (Link l : ilinks) {
			if (l.getDestinationPort() != null && l.getVariable() != null) {
				...
				portBindings.put(l.getDestinationPort(), (Binding)SerializableObjectCloner.clone(l.getVariable().getBinding()));
	...
	public static PortBindingList handlePortSetRule(SetExpression expr, SetExpression default_expr, 
		...
		PortBindingList tmpBindings = getPossiblePortBindings(expr, modPortBindings(expr, initPortBinding), finalPortBindings);
		int numext = 1;
		while (numext > 0) {
			numext = 0;
			...
			for (int i = 0; i < len; i++) {
				...
				if (!b.isList()) {
					...
					if (f) {
						numext++;
						// FIXME: Replaced expr with default_expr (X Product) on second iteration ! (Testing phase)
						PortBindingList tmp = handlePortSetRule(default_expr, default_expr, b.getPortBinding(), new PortBindingList());
			...
			for (SetExpression cexpr : expr) {
				...
				if (expr.getOperator() == SetOperator.REDUCEDIM && !cexpr.isSet()) {
					...
					if(b.isSet() && b.size() > 0) {
						...
						// If other items in the collection are also sets, then add their elements
	...
	private static PortBindingList getPossiblePortBindings(SetExpression expr, PortBinding portBindings, PortBindingList finalPortBindings) {
		if (expr.isSet()) {
			for (SetExpression cexpr : expr) {
				PortBindingList cbindings = getPossiblePortBindings(cexpr, portBindings, new PortBindingList());
				...
				if (expr.getOperator() == SetOperator.NWISE) {
					...
					for (int i = 0; i < finalPortBindings.size(); i++)
						finalPortBindings.get(i).getPortBinding().putAll(cbindings.get(i).getPortBinding());
					...
					for (PortBindingList cbinding : cbindings) {
						if (i > 0) {
							for (int j = 0; j < num; j++) {
								finalPortBindings.add(new PortBindingList(new PortBinding(finalPortBindings.get(j).getPortBinding())));
							}
							index += num;
						}
						i++;
						for (int j = index; j < finalPortBindings.size(); j++) {
							finalPortBindings.get(j).getPortBinding().putAll(cbinding.getPortBinding());
						}
					}
				} 
			}
		...
		if (pblist.isList()) {
			...
			for (PortBindingList pbl : pblist) {
				...
				for (Port p : cpb.keySet()) {
					...
					if (b == null) {
						b = new Binding();
						b.setMetrics(cb.getMetrics());
					}
					else {
						// ****************** TODO: Remove Non-Matching metrics from the new Binding **********************
					}
					/*if (cb.isSet() && cb.size() == 1) {
						b.addAll(cb);
					} else {
						b.add(cb);
					}*/
					// Temporarily replaced the above block with the one line below (Testing phase)
					b.add(cb);					
					pb.put(p, b);
				}
			}
